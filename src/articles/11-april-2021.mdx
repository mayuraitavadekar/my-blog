In this article, I am going to write my personal experiences with JS. Since the last 3 years,
...
In my honest opinion, it is <b>not a mature programming language</b> like C++/Java/Go. The language rapidly grew.
...
Some people write Data Structures and Algorithms in JS. I too wrote. But I don't understand it. If you are writing it for your own understanding then it is okay.
But there is no way JS is ideal to write DS&ALGO. Systems are critical and you must write systems in C++. The whole point of learning algorithms is to understand
...
are written in React. I like the community. It helps to solve the problems quickly. In the industry where to giants(google & facebook) open sourced there frameworks, Evan You single handedly worked on his framework: Vue.

<br />I love it. Simple, efficient and Reliable. Lot of big companies, Unicorn
...
Depends. If you want to write back-end for light weight software where you won't receive that much requests, if you are not building something where end-user
cannot bear the waiting time from server-side. Then you are good to go with JS backend. Use fastify(recommended), express.

<br />
In some cases where it is not okay if user is not getting response immedietly after
sending request to server, it is not ideal to user JS back-end. Then what you should
use in such case: Golang. My personal recommendation to write back-ends. Else use
Java. Java and Golang are used heavily by many unicorn startups and big tech giants
to write critical servers.

<br />I personally wrote few thousands lines of server-side JS. It is working
...
<br />
If you know React and want to build a light weight app - use React Native. My personal
