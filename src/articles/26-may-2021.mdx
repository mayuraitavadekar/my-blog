Since last couple of months, I have been working on my project. Now, one of the reason why I am writing this article is because I want to keep some personal notes
on which steps we went through while building a MVP which needs to be efficient in every aspect.

<br />

We constantly tried a lot of things, especially on server-side. It is really difficult to choose what to use. You must try different things and check whether it
suits your need or not. In almost every software, choosing right trade off is extremely important skill and I have learned it the hard way. Let's begin.
...
received single problem till date.
...
We intentionally avoid data intensive operations such as pagination, filteration on client side simply because:
 - We don't need it. we strictly kept it as simple to use as possible.
 - It unnecessarily increases cost for the sake of user experience. 

<br />

All operations are CRUD operations, with zero computations on client side as well on server side.
